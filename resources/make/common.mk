...
ERL_LIBS=.:..:../pattern-match-util:$(shell $(LFETOOL) info erllibs)
OS := $(shell uname -s)
ifeq ($(OS),Linux)
        HOST=$(HOSTNAME)
endif
...
get-version:
	...
	-eval "lfe_io:format(\"~p~n\",['pattern-match-util-util':'get-versions'()])." \
...
install: compile
	@echo "Installing pattern-match-util ..."
	@PATH=$(SCRIPT_PATH) lfetool install lfe

upload: $(EXPM) get-version
	@echo "Preparing to upload pattern-match-util ..."
	@echo
	@echo "Package file:"
	@echo
	@cat package.exs
